...
function dailyAnime() {
  return state.pool[hash('anidle:' + mskDateStr()) % state.pool.length];
}
...
function randomAnime() {
  let a;
  do { a = state.pool[Math.floor(Math.random() * state.pool.length)]; }
  while (state.anime && a.id === state.anime.id && state.pool.length > 1);
  return a;
}
